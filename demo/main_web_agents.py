...
def main():
    ...
    parser = argparse.ArgumentParser(description="启动支持智能体切换的Youtu-Agent WebUI")
    parser.add_argument(
        "--config", type=str, default=DEFAULT_CONFIG, help="默认智能体配置文件 (例如: examples/svg_generator.yaml)"
    )
    parser.add_argument("--ip", type=str, default=DEFAULT_IP, help="监听IP地址")
    parser.add_argument("--port", type=int, default=DEFAULT_PORT, help="监听端口")
    parser.add_argument("--autoload", action="store_true", default=DEFAULT_AUTOLOAD, help="启用自动重载")

    args = parser.parse_args()

    print("🚀 启动Youtu-Agent WebUI...")
    print("📱 界面类型: WebUIAgents (支持智能体切换)")
